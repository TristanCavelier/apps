...
(function (exports) {
  ...
  ExtendedStreamableChain.prototype.toLines = function () {
    var ecc = new ExtendedCancellableChain(), lines = [], remaining = "";
    return this.toTexts().stream(function rec(stream) {
      if (lines === null) {
        return Promise.resolve({"done": true});
      }
      if (lines.length > 0) {
        return Promise.resolve({value: lines.shift()});
      }
      return ecc.call(stream, stream.next).then(function (next) {
        var i;
        if (next.done) {
