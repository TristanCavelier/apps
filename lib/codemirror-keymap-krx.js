...
//   }
  ...
  function autocompleteWord(cm, reverse) {
    var vars = cm.krxAutocompleteWordVars;
    var cursor = cm.getCursor();
    var words = [], wordsDict = {}, index, cursor2;
    var firstText, lastText, wordPart;

    function add(word) {
      if (word.startsWith(wordPart) && word !== wordPart && wordsDict[word] === undefined) {
        wordsDict[word] = 1;
        words.push(word);
      }
    }
    ...
    if (vars && vars.location && vars.location.line === cursor.line && vars.location.ch === cursor.ch && vars.words && cm.getLine(cursor.line).slice(0, cursor.ch).endsWith(vars.words[vars.index || 0])) {
      ...
      index = vars.index;
      if (reverse) {
        if (index < vars.words.length - 1) { index += 1; }
        else { index = 0; }
      } else {
        if (index > 0) { index -= 1; }
        else { index = vars.words.length - 1 }
      }
      cm.off("change", autocompleteWordOnChangeListener);
      ...
      lastText.replace(/\w+/g, add);
      firstText.replace(/\w+/g, add);
  ...
  CodeMirror.keyMap.krx["Alt-\\"] = removeSpacesAtCursor;
  // CodeMirror.keyMap.krx["Alt--"] = function (cm) {
  ...
  CodeMirror.keyMap.krx["Alt-Space"] = "krxAutocompleteWord";
  ...
  CodeMirror.keyMap.krx["Shift-Alt-V"] = doNothing; // TODO selectLine (firefox [View] shortcut)
